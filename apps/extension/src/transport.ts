...
export class ListingUploadQueue {
  ...
  public enqueue(listings: readonly MarketplaceListing[]): Promise<void> {
    this.#operation = this.#operation.then(async () => {
      const pending = await this.pending();
      const byKey = new Map(pending.map((listing) => [listingKey(listing), listing]));
  ...
  public async pending(): Promise<MarketplaceListing[]> {
    return (await this.#sessionStorage.get<MarketplaceListing[]>(PENDING_KEY)) ?? [];
  }
  ...
  public flush(): Promise<void> {
    this.#operation = this.#operation.then(async () => {
      ...
      const connection = await this.#localStorage.get<ConnectionSettings>(CONNECTION_KEY);
      ...
      try {
        ...
        if (!response.ok) throw new Error(`listing ingest returned ${response.status}`);
        ...
        console.warn('MarketScope server is unavailable; listings remain queued', error);
        this.#schedule(RETRY_DELAY_MS);
      }
    });
    return this.#operation;
...
export async function getExtensionState(
  ...
  if (connection !== undefined) {
    try {
      ...
      if (!response.ok) throw new Error(`watchlist request returned ${response.status}`);
      ...
      console.warn('MarketScope is using cached watchlists while offline', error);
    }
  }
