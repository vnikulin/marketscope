...
describe('PWA API', () => {
    ...
    const response = await testServer.server.app.inject({
      ...
      payload: {
        listings: [
          ...
          listing('pwa-block', {
            title: 'Garmin case',
            rawText: 'Garmin case $120 Freeport, NY',
          }),
        ],
      },
    });
  ...
  test('projects matches, blocked listings, favorites, dashboard, and diagnostics', async () => {
    const matches = await testServer.server.app.inject({
      method: 'GET',
      url: '/api/listings?view=matches',
      headers: adminHeaders(session),
    });
    expect(matches.statusCode).toBe(200);
    const matchedListings = matches.json<{
      listings: Array<{
        id: string;
        title: string;
        evaluations: Array<{ verdict: { passed: boolean } }>;
      }>;
    }>().listings;
    ...
    const blocked = await testServer.server.app.inject({
      method: 'GET',
      url: '/api/listings?view=blocked',
      headers: adminHeaders(session),
    });
    ...
    const favorite = await testServer.server.app.inject({
      method: 'PUT',
      url: `/api/listings/${listingId}/favorite`,
      headers: adminHeaders(session),
    });
    expect(favorite.statusCode).toBe(200);
    const favorites = await testServer.server.app.inject({
      method: 'GET',
      url: '/api/listings?view=favorites',
      headers: adminHeaders(session),
    });
    expect(favorites.json<{ listings: unknown[] }>().listings).toHaveLength(1);

    const dashboard = await testServer.server.app.inject({
      method: 'GET',
      url: '/api/dashboard',
      headers: adminHeaders(session),
    });
    expect(
      dashboard.json<{
        ...
        recentListings: Array<{ title: string }>;
      }>(),
    ).toMatchObject({
      matches: 1,
      blocked: 1,
      favorites: 1,
      matchesLast24Hours: 1,
      recentListings: [{ title: 'Garmin GPSMAP 1042xsv' }],
    });
    const diagnostics = await testServer.server.app.inject({
      method: 'GET',
      url: '/api/diagnostics',
      headers: adminHeaders(session),
    });
    expect(diagnostics.json()).toMatchObject({
      ...
      listingsLast24Hours: 2,
      notificationQueueDepth: 0,
    });
  });

  test('backs up without SMTP secrets and restores after a validated snapshot', async () => {
    const listings = await testServer.server.app.inject({
      method: 'GET',
      url: '/api/listings?view=history',
      headers: adminHeaders(session),
    });
    const favoriteId = listings.json<{ listings: Array<{ id: string }> }>()
      .listings[0]?.id;
    expect(favoriteId).toBeDefined();
    await testServer.server.app.inject({
      method: 'PUT',
      url: `/api/listings/${favoriteId}/favorite`,
      headers: adminHeaders(session),
    });
    ...
    const invalid = await testServer.server.app.inject({
      method: 'POST',
      url: '/api/restore',
      headers: adminHeaders(session),
      payload: { version: 99 },
    ...
    const restored = await testServer.server.app.inject({
      method: 'POST',
      url: '/api/restore',
      headers: adminHeaders(session),
      payload: backup,
    ...
    expect(
      existsSync(join(testServer.directory, 'backups', result.snapshot)),
    ).toBe(true);
    const history = await testServer.server.app.inject({
      method: 'GET',
      url: '/api/listings?view=history',
      headers: adminHeaders(session),
    });
